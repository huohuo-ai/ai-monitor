...
def wechat_open_login():
    ...
    # 生成state参数
    state = secrets.token_urlsafe(16)
    session['wechat_open_state'] = state
    
    # 构建微信开放平台扫码登录URL
    params = {
        'appid': WECHAT_OPEN_CONFIG['app_id'],
        'redirect_uri': WECHAT_OPEN_CONFIG['redirect_uri'],
        'response_type': 'code',
        'scope': 'snsapi_login',  # 扫码登录固定作用域
        'state': state
    }
...
@app.route('/api/auth/wechat/open/callback', methods=['GET'])
def wechat_open_callback():
    """微信开放平台扫码登录回调"""
    code = request.args.get('code')
    state = request.args.get('state')
    
    # 验证state参数
    if not state or state != session.get('wechat_open_state'):
        return jsonify({'error': '无效的请求'}), 400
    ...
    try:
        ...
        token_params = {
            'appid': WECHAT_OPEN_CONFIG['app_id'],
            'secret': WECHAT_OPEN_CONFIG['app_secret'],
            'code': code,
            'grant_type': 'authorization_code'
        }
        ...
        openid = token_data.get('openid')
        access_token = token_data.get('access_token')
        unionid = token_data.get('unionid')  # 开放平台可能有unionid
        ...
        # 第三步：查找或创建用户
        # 优先使用unionid查找（如果有的话），否则使用openid
        lookup_id = unionid or openid
        user = db.get_user_by_wechat_openid(lookup_id)
        
        if not user:
            # 创建新用户
            user_id = db.create_user(
                wechat_openid=lookup_id,
                wechat_nickname=userinfo.get('nickname'),
                wechat_headimgurl=userinfo.get('headimgurl')
            )
            is_new = True
        ...
        # 设置session
        session['user_id'] = user_id
        session['login_type'] = 'wechat_open'
        session.permanent = True
        app.permanent_session_lifetime = timedelta(days=7)
